...
def dro_lyap_search_for_gd(mu, L, eta, n_points, eps_val=1e-4) :
    ...
    # Generate samples
    _, _, avgG, avgF = sample_generation(iter_K=n_points, N=100, mu=mu, L=L)
    ...
    if n_points > 0 :
        ...
        for k in range(n_points) :
            Pk1 = np.vstack([repX[k]-repX[s], repG[k], repG[k+1]])
            constraints += [
                Vk_G[k,:,:] - Vk_G[k+1,:,:] \
                    + lmbd[k, 0] * A_list[idx_list.index((k,k+1))] \
                    + lmbd[k, 1] * A_list[idx_list.index((s,k))] \
                ...
                Vk_F[k,:] - Vk_F[k+1,:] \
                    ...
                    # + lmbd[k, 2] * b_list[idx_list.index((s,k+1))] \
                    + Y[k, :] == 0,
                X[k,:,:] == Pk1.T @ cp.Variable((3, 3), symmetric=True) @ Pk1,
    ...
    YK = cp.Variable(dimF)
    PK = np.vstack([repX[n_points]-repX[s], repG[n_points]])
    ...
    constraints += [
        ...
        Vk_F[n_points,:] - V_obj_F \
            ...
            + YK == 0,
        XK == PK.T @ cp.Variable((2, 2), symmetric=True) @ PK,
...
if __name__ == "__main__":
    ...
    fig.suptitle("Smooth convex minimization: GD")

    # First subplot: Primal and Dual PEP
    axs[0].plot(range(1,iter_K), L/2/(2*np.arange(1,iter_K)+1), '^--', label='$L/(4K+2)$')    # primary
    ...
    plt.savefig('results/gd_pep_results_loglog.pdf', dpi=300)
