...
lyap_params = sdpvar(2,1); cons = cons + (lyap_params >= 0);
cons = cons + (sum(lyap_params) == 1);
...
PSD_dual = tau_tested * ( lyap_params(1) * (z0-zs)'*(z0-zs)  + lyap_params(2) * (y0-ys)'*(y0-ys));
LIN_dual = 0 * fs;
...
Aij_obj = lyap_params(1) * (zk(end,:)-zs)'*(zk(end,:)-zs)  + lyap_params(2) * (yk(end,:)-ys)'*(yk(end,:)-ys);
...
% status = optimize(cons,0,options);
% status = optimize(cons,-lyap_params(1),options);
status = optimize(cons,-lyap_params(2),options);
