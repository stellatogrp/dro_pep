...
from sdp_data_matrix_scalable import smooth_convex_gd_close_to_sample, generate_sample
...
def inf_gap(R):
    ...
    for i in range(length):
        # gap = gap.at[i].set(jnp.trace(A[i]@G_full) - b[i])
...
def Lagrangian(R, y):
    ...
    lagrangian = jnp.trace(S.G@PTP_full) + jnp.sum(S.F*(R.f**2)) + jnp.sum(S.S*(R.s**2))
...
def augmented_Lagrangian(R, y, sigma):
    gap = inf_gap(R)
    PTP_full = PTP_full_form(R)
    # lagrangian = jnp.trace(C@R@R.T) # pass the R-multiplication itself / calculate trace(..) by itself (no R formation)
...
#              [       O         eye(K+1)     O    ]
#              [       O            O       eye(m) ]
mask = np.zeros(((K+1)+d+(K+1)+m, d+(K+1)+m))
mask[:K+1,:d] = 1.0
...
while not converged:
    ...
    if (term_check < term_crit and v < term_crit) or (k > 100):
        converged = True
        if k > 100 :
            print('[Iter max over] Reached the maximum iteration number for augmented Lagrangian')
...
# np.save(f'fseed=0/seed=2/bm_data_d={d}_K={K}_radius={radius}.npy', data_dict)