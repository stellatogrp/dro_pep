"""
Quad median-vs-mean summary (no plots).
...
def write_markdown(data, training_sample_N, md_path):
    lines = []
    lines.append('# Quad — mean vs. median final-iterate loss')
    lines.append('')
    lines.append(
...
def main():
    ...
    for key, cfg_list in groups.items():
        ...
        print(f'  Saved {md_path.relative_to(quad_dir)}')
