#!/usr/bin/env python3
"""Paper-style LogReg (german.numer) figures for GD.

Writes into iclr_data_outputs/figures/:
    logreg_gd_losses.pdf / .csv                  in-dist | OOD, mean + [q10,q90]
    logreg_gd_frac_problems_solved.pdf / .csv    2 x 3 grid over eta
    logreg_gd_times.csv                          mean +/- 2 sigma per SGD iter
...
Usage (from anywhere):
    python plot_logreg_gd.py
    python plot_logreg_gd.py --data-dir DIR --runs-root DIR --out-dir DIR
"""
import argparse
import sys
...
def main():
    ...
    args = p.parse_args()
    logreg_figures.build('gd', runs_root=args.runs_root, data_dir=args.data_dir,
