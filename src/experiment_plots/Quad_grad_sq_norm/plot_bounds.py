...
def compute_exp_prob(samples, pep, dro, k):
    ...
    for g in range(groups):
        ...
        mean = samples_g['grad_sq_norm'].mean()
...
def compute_empirical_avg(samples, k):
    return samples[samples['K'] == k]['grad_sq_norm'].mean()
...
def compute_cvar_prob(samples, pep, dro, k, alpha=0.1):
    ...
    for g in range(groups):
        ...
        samples_g = samples[(idx_low <= samples['i']) & (samples['i'] < idx_high) & (samples['K'] == k)]
        quantile = samples_g['grad_sq_norm'].quantile(1-alpha)
        tail_loss = samples_g[samples_g['grad_sq_norm'] >= quantile]
        cvar = tail_loss['grad_sq_norm'].mean()
...
def compute_empirical_cvar(samples, k, alpha=0.1):
    ...
    quantile = samples_k['grad_sq_norm'].quantile(1-alpha)
    tail_loss = samples_k[samples_k['grad_sq_norm'] >= quantile]
    return tail_loss['grad_sq_norm'].mean()
...
def main_bounds():
    ...
    GD_cvar_dro_eps = GD_cvar_dro[GD_cvar_dro['eps_idx'] == 2]
    ...
    GD_worst_cases = GD_samples[['K', 'grad_sq_norm']].groupby(['K']).max()
    NGD_worst_cases = NGD_samples[['K', 'grad_sq_norm']].groupby(['K']).max()
    ...
    ax[0].plot(range(1, exp_K_max + 1), GD_pep[GD_pep['obj'] == 'grad_sq_norm']['val'][:exp_K_max], label='GD', color=GD_color)
    ...
    ax[0].plot(range(1, exp_K_max + 1), NGD_pep[NGD_pep['obj'] == 'grad_sq_norm']['val'][:exp_K_max], label='AGD', color=NGD_color)
    ...
    plt.suptitle('Quadratic Minimization, Squared Gradient Norm')

    # plt.show()
    plt.savefig(f'quad_grad_sq_norm.pdf')
