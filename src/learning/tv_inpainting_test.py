...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("Generating second-row problem (same face, different mask, white missing)")
    print("=" * 60)

    problem2 = generate_tv_inpainting_problem(
        missing_fraction=MISSING_FRACTION_OOD,
        random_seed=43,
        face_index=202,
    )
    S2 = len(problem2["known_indices"])
    extractor2 = make_matrix_extractor(problem2["known_indices"], M, N)
    matrices2 = extractor2(known_values=problem2["known_values"])
    solution2 = solve_lp(matrices2)
    print(f"[row 2] S={S2}, optimal objective: {solution2['objective_value']:.4f}")
    ...
    miss_pct_ood = int(round(MISSING_FRACTION_OOD * 100))
    titles_row2 = [
        None,
