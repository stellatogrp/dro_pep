...
def main_lasso_cvar():
    ...
    Lasso_ista_cvar_data = Lasso_ista_cvar[Lasso_ista_cvar['eps_idx'] == 2]
    Lasso_fista_cvar = pd.read_csv(f'Lasso/dro/FISTA_cvar_1_25/dro.csv')
    Lasso_fista_cvar_data = Lasso_fista_cvar[Lasso_fista_cvar['eps_idx'] == 2]

    sublinear_curve_fit(
        Lasso_ista_cvar_data,
        csv_fname='Lasso/ista_cvar_fit_params.csv',
        plot_fname='Lasso/ista_cvar_fit_plot.pdf',
        mincutoff=15,
    )

    linear_curve_fit(
        Lasso_fista_cvar_data,
        csv_fname='Lasso/fista_cvar_fit_params.csv',
        plot_fname='Lasso/fista_cvar_fit_plot.pdf',
        mincutoff=15,
    )


def main_strongcvx_ngd_test():
    df = pd.read_csv('Quad_StrongCvx/samples.csv')
    worst_cases = df[['K', 'obj_val']].groupby(['K']).max().reset_index()
    worst_cases = worst_cases.rename(columns={'obj_val': 'dro_feas_sol'})
    print(worst_cases)
    linear_curve_fit(
        worst_cases,
        csv_fname='Quad_StrongCvx/sample_fit_params.csv',
...
def main_lasso_test():
    df = pd.read_csv('Lasso/samples.csv')
    worst_cases = df[['K', 'obj_val']].groupby(['K']).max().reset_index()
    worst_cases = worst_cases.rename(columns={'obj_val': 'dro_feas_sol'})
    print(worst_cases)
    # plt.plot(range(1, 26), worst_cases)
    # plt.yscale('log')
    ...
    linear_curve_fit(
        worst_cases,
        csv_fname='Lasso/sample_fit_params.csv',
        plot_fname='Lasso/sample_fit_plot.pdf',
        mincutoff=15,
    )
